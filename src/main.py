...
################## <--classic classification problem + reconstruction--> ###################
    ...
    if pdct == "class": # used for doing a std classify problem vs. prediction problem (only Sig2XYZ right now)
        ...
        if mode == "generate":
            ...
            loss_fn = nn.MSELoss() 
            optimizer = "adam"
            generate(input_shape=in_layer, latent_dims = 20, loss_fn = loss_fn, optimizer = optimizer, train_loader = train_dataset, val_loader = val_dataset, test_loader= test_dataset, epochs = epochs, model = model) # note model here is standard FFNN used for predicting generated knot type
