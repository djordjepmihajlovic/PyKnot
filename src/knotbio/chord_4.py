...
def chord_combinatorical_STS(STS):
    ...
    for idy in range(0, samples): # samples
        ...
        chord_data_1.append(integral1 / (100 * 100 * 100 * 100))
        chord_data_2.append(integral2 / (100 * 100 * 100 * 100))
        chord_data_3.append(integral3 / (100 * 100 * 100 * 100))
        chord_data_4.append(integral4 / (100 * 100 * 100 * 100))
        chord_data_5.append(integral5 / (100 * 100 * 100 * 100))
        chord_data_6.append(integral6 / (100 * 100 * 100 * 100))

    return chord_data_1, chord_data_2, chord_data_3, chord_data_4, chord_data_5, chord_data_6
...
def main():
    ...
    for x in knots:
        ...
        chord_1, chord_2, chord_3, chord_4, chord_5, chord_6 = chord_combinatorical_STS(STS)

        with open(f'{x}_A4_1.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            for item in chord_1:
                writer.writerow([item])

        with open(f'{x}_A4_2.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            for item in chord_2:
                writer.writerow([item])

        with open(f'{x}_A4_3.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            for item in chord_3:
                writer.writerow([item])

        with open(f'{x}_A4_4.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            for item in chord_4:
                writer.writerow([item])

        with open(f'{x}_A4_5.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            for item in chord_5:
                writer.writerow([item])

        with open(f'{x}_A4_6.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            for item in chord_6:
                writer.writerow([item])
